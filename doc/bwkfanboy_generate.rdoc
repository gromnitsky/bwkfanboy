...
bwkfanboy_generate--[TODO: write a summary here.]


==Synopsis

 bwkfanboy_generate [options]


==Description

The bwkfanboy_generate utility [TODO: write a description here.]
...
bwkfanboy_generate looks for its configuration at 3 places at start up.

1. At <tt>BWKFANBOY_CONF</tt> env variable.
   (Its format is exactly similar to CL options.)

2. At the configuration file. Its default name is
   <tt>bwkfanboy.yaml</tt> and it can be stored in several
...
gemdir`/gems/bwkfanboy-x.y.z/etc/</tt> directory for samples.


==Examples

 % bwkfanboy_generate --config-dirs
 % bwkfanboy_generate -V
